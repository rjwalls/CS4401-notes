...
### Page Alignment and Partial Overwrites

One important aspect of ASLR is that it must respect page boundaries. Memory pages (typically 4096 bytes or 0x1000 in size) are the basic unit of memory management for the operating system. This means that randomized addresses will always be aligned to page boundaries.

As a result, the least significant 12 bits (3 hexadecimal nibbles) of any address will remain consistent even with ASLR enabled. All sections randomized by ASLR will start at addresses ending with `000` because of this page alignment requirement.

This feature can be exploited through **partial overwrites**. Instead of overwriting an entire address (which would require knowing the randomized high-order bits), an attacker can:

1. Overwrite only the least significant bytes of an address
2. Keep the randomized high-order bits intact
3. Change just enough to redirect execution to a desired location

For example, if a function's address is `0x7ffff7a12774` and the target is `0x7ffff7a13580`, the attacker only needs to overwrite the last 2 bytes (`0x774` → `0x580`). The randomized portion remains unchanged.

With a stack-based buffer overflow, the attacker can perform this partial overwrite because:

1. The overwrite occurs one byte at a time from lower to higher addresses
2. Due to little-endian byte ordering, the least significant bytes come first
3. The attacker can stop the overflow after overwriting just the necessary bytes

This technique is especially useful in challenges where:

- ASLR is enabled but you can leak a pointer to one section
- You have a limited buffer overflow that can only modify a few bytes
- You're trying to transform one address (like a return address) into another address in the same section

In certain exploitation scenarios, you might only need to guess a single byte, which gives you only 256 possibilities to try - making brute force approaches practical even with ASLR enabled.

### Working Directory Considerations

When exploiting programs with ASLR enabled, the working directory from which
you execute your exploit can affect stack addresses because argument and
environment strings contribute to the process layout. Keep the working
directory and launch method consistent when an exploit depends on stack
addresses.

`stack2g-64` does not rely on a stable stack address. It leaks the runtime
address of one function, uses fixed offsets within the PIE executable to derive
another function, and sends the resulting overwrite to that **same process**.
Changing the working directory does not change the difference between two
symbols in one ELF. Starting a new process does change the randomized PIE base,
so a leak from an earlier execution cannot be reused blindly.
