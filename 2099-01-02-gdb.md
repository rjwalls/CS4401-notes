...
know! The course shell server loads
[GEF](https://hugsy.github.io/gef/) by default, which makes GDB friendlier for
exploit development and binary analysis. If you build your own local
environment later, install GEF if you want the same commands and output shown
in class.
...
**GEF quick reference:**
 - `context` redraws the GEF register, stack, code, and backtrace panes.
 - `checksec` shows binary protections such as NX, PIE, RELRO, and stack
   canaries. You can also run `checksec ./binary` before starting the program.
 - `vmmap` shows the process memory map and page permissions.
 - `xinfo <address>` explains where an address lives and what permissions that
   page has. Registers work here too, e.g., `xinfo $rsp`.
 - `pattern create <length>` creates a cyclic pattern for finding overwrite
   offsets, and `pattern search <value>` finds the matching offset.
 - `search-pattern <value>` searches process memory for a value or address.
...
**Stepping:**
 ...
 - `finish` will execute until the current function returns
...
**Getting information:**
 ...
 - `aslr` will tell you if ASLR is enabled from GDB's perspective. Note that the
   output may be different before running the binary. You can disable ASLR with
   `aslr off`.
 - Use `vmmap` to view memory regions and permissions.
