...
Below is a list of topics and lecture materials for this course. It's quite an extensive list, and it's likely we will not have sufficient time to cover everything. To help you prioritize your efforts, please note that any materials marked as "required" will be most useful in helping you solve challenges and earn points. Therefore, it's advisable to focus on those materials first. Finally, the preliminary set and order of released challenges will vary from what is shown below. Depending on how the course is going, I may release additional challenges, remove some of the challenges listed here, or change the order in which they are released.
...
set of notes walks through the practical setup you should complete before the
first lecture, including connecting to the course shell server, configuring your
shell account, installing Ghidra, and understanding when a local CTF environment
is useful.
...
`stack0g-64`, `stack1g-64`, and `stack2g-64` lecture challenges.
 -  [Lecture Notes: ASLR][notes_aslr]. Read the introductory discussion of PIE
and information leaks before `stack2g-64`.
 - [Lecture Notes and Mini-Lab: Ghidra
   Fundamentals.](http://cs4401.walls.ninja/notes/lecture/ghidra.html) Complete
this after Stack 2 and before Warp Drive. It covers the analysis workflow you
will need when a companion challenge provides a binary but withholds source.
...
the tool you will use the most in this course. 
 ...
 - [GEF documentation.](https://hugsy.github.io/gef/) GEF is a very useful
   extension to GDB that is installed on the course shell server.
...
up a bit. This will be useful for solving `stack1`.
 ...
 - [Darknet Diaries][darknet_diaries]. This podcast adds a more human,
   story-driven angle to security incidents and the people around them.


**Related challenges:**
 - Lecture Sequence: `stack0g-64`, `stack1g-64`,
   `stack2g-64`.
 - Sequence: `bandit0-12` introductory level, `bandit13-20` basic level, `bandit21-33` basic level
 - After Stack 0: `stack0g-guessdown`, basic level.
 - After Stack 2: complete the Ghidra mini-lab, then attempt
   `stack2g-warp-drive`, basic level.

The Stack 0-2 binaries and companions are built in a pinned Docker environment
so their x86-64 layouts stay reproducible. They are ordinary shell challenges
at runtime; students do not launch Docker containers to solve them.
...
highlight some of the conventions of 64-bit binaries.     
 ...
 - [Pwntools Documentation][pwn_docs]. The official docs are the current
   reference for writing bytes correctly, generating shellcode, working with
   core files, and connecting to remote services when those features come up.
 

**Related challenges:**
 - Lecture Sequence: `stack4r-64`, `stack4r-32`
 - `Revenge of stack4` intermediate level
 - `stack4r-life-support` intermediate level
...
**Related challenges:**
 ...
 - Lecture Sequence: `stack6r-d`
 - `wargames`, intermediate level.
 - `wargames joshua`, intermediate level.
...
**Related challenges:**
 ...
 - `the-bar` **This challenge must be
   solved in order to pass the course!**

### Topic: In-Depth Topics and Defenses

The topics below are likely to be sprinkled throughout the course rather than
covered as one neat final unit. As we encounter stronger mitigations, unfamiliar
programs, and less direct exploit paths, we will return to these ideas and use
them to connect earlier techniques to more advanced attacks and defenses.
...
constructing the ROP chain.  
 ...
 - ["Hacking Blind."][hacking_blind] by Bittau et al. This paper shows how to construct a ROP
   attack *without* access to the binary, i.e., a Blind ROP attack. You should not read every section
...
**Related challenges:**
 ...
 - `dog-house`, intermediate level
 - `Shadow Strike`, intermediate level
...
[darknet_diaries]:https://darknetdiaries.com/
