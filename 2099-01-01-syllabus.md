...
**Challenge Sequences.** Some challenges will only be unlocked after you have solved the preceding challenges in the sequence. For example, the first challenge sequence is `stack0r-64`, `stack1r-64`, `stack2r-64`, and `stack3r-64`. Challenges in a sequence typically unlock in order, e.g., you must solve `stack0r-64` to unlock `stack1r-64`. Also, challenges in a sequence typically have the same due date. 

**Challenge Unlocking.** In addition to the sequences described above, some challenges have other prerequisites which must be met for the challenge to unlock. For example, some challenges only unlock when the student has earned enough points. Other challenges will only unlock after a certain date. Other challenges only unlock under mysterious circumstances. 
...
Similarly, there are some challenges which students must solve to achieve an
'A', regardless of their point total. Currently, there is only one such
challenge:

 - Either `stockholm-syndrome` or `hackerman` must be solved to get an 'A'.
   Both of these challenges are listed under the `Participation` tab and
involve creating a new challenge for the course. 
...
of the username that you have selected.** **Students must also join the `CS557`
classroom (owner is `ctfadmin`) on the course webpage.** If a student does not
adhere to these two requirements, the instructor will not be able to track the
student's progress and, thus, they will not receive a grade.
...
exploitation. Further, CS557 has greater point cutoffs for letter grades and
